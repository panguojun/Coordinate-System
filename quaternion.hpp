/**************************************************************************
    ...
    for rotation operations. Vectors are derived from quaternions, but there 
    are differences between the two. Currently, there is some debate regarding 
    the relationship and applications between quaternions and vectors.

    *  *  *  *  *  *  *  *  *  *  *  Detailed Explanation  *  *  *  *  *  *  *  *  *  *  *  *  * 
    Similar to complex numbers, quaternions also have an exponential form: e^q, 
    which results in another quaternion: q = e^q. In fundamental physics, this 
    corresponds to gauge transformations, which differ from coordinate transformations; 
    gauge transformations focus more on the time attribute of phase, while 
    coordinate transformations lean towards spatial changes and curvature extraction.

    Quaternions support operations such as normalization, conjugation, inversion, 
    multiplication, and division, and they define a unit quaternion (ONE).
**************************************************************************/
struct quaternion
{
    ...
    {
        // Implementation based on Nvidia SDK
        vector3 uv, uuv;
        vector3 qvec(x, y, z);
        uv = qvec.cross(v);
        uuv = qvec.cross(uv);
    ...
    friend void operator*= (vector3& v, const quaternion& q)
    {
        v = q * v;
    }
    
    // This is the implementation from the Ogre engine (parent-child order)
    quaternion operator*(const quaternion& rkQ) const
    {
    ...
    // Exponential operation (note the operator precedence)
    ...
    #define fromvectors from_vectors // Alias
    quaternion from_vectors(const vec3& v1, const vec3& v2)
    {
        ...
        {
            real dot = v1.dot(v2);
            if (dot == -1.0) // Handle the 180-degree case
            {
                vec3 ax;
                vec3 uz = vec3::UZ;
                if (v1.x < EPSILON && v1.x > -EPSILON && v1.y < EPSILON && v1.y > -EPSILON)
                    uz = vec3::UX;
    ...
    //-----------------------------------------------------------------------
    // Defined by angle and axis
    quaternion ang_axis(real rfAngle, const vec3& rkAxis)
    {
        // Assert: axis[] is of unit length
        //
    ...
    //-----------------------------------------------------------------------
    #define fromeulers from_eulers // Alias
    ...
    //-----------------------------------------------------------------------
    // roll, pitch, yaw
    #define toeulers to_eulers // Alias
    ...
    //-----------------------------------------------------------------------
    static quaternion slerp(real fT, const quaternion& rkP, const quaternion& rkQ, bool shortestPath)
    {
        ...
        else
        {
            ...
            // 1. "rkP" and "rkQ" are very close (fCos ~= +1), so we can do linear
            //    interpolation safely.
            // 2. "rkP" and "rkQ" are almost inverse of each other (fCos ~= -1), there
            //    are an infinite number of possibilities for interpolation. But we haven't
            //    developed a method to fix this case, so just use linear interpolation here.
    ...
    //-----------------------------------------------------------------------
    static quaternion nlerp(real fT, const quaternion& rkP, const quaternion& rkQ, bool shortestPath)
    {
        quaternion result;
        real fCos = rkP.dot(rkQ);
        if (fCos < 0.0f && shortestPath)
    ...
    quaternion spherical_cubic_interpolate(const quaternion& p_b, const quaternion& p_pre_a, const quaternion& p_post_b, const real& p_weight) const {
        ...
        // Calculate by Expmap in from_q space.
        ...
        // Calculate by Expmap in to_q space.
