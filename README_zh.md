...
```
K = R_{1212} / det(g) = -⟨Ω_uv r_v, r_u⟩ / √det(g)
```

其中：
- `r_u = ∂r/∂u`, `r_v = ∂r/∂v`：坐标基向量
- `det(g) = ‖r_u‖²‖r_v‖² - ⟨r_u, r_v⟩²`：度量张量行列式
- `√det(g)`：面积元缩放因子

### 理论验证

**球面验证**（理论曲率 K=1）：
- 计算得：`⟨Ω_uv r_v, r_u⟩ = -sinθ`
- `√det(g) = sinθ`
- `K = -(-sinθ)/sinθ = 1` ✓
